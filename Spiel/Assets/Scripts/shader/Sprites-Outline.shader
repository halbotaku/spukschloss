...
#include "UnityCG.cginc"
	...
	if (_Outline > 0 && c.a != 0) {
		...
		if (IN.texcoord.x < _Rect.x + _OutlineSize*_MainTex_TexelSize.x || IN.texcoord.y < _Rect.y + _OutlineSize*_MainTex_TexelSize.y ||
			IN.texcoord.x > _Rect.z - _OutlineSize*_MainTex_TexelSize.x || IN.texcoord.y > _Rect.w - _OutlineSize*_MainTex_TexelSize.y)
		{
			totalAlpha = 0;
		}
		else
